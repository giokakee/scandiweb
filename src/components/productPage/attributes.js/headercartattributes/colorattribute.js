  ...
  render() {
    ...
    return (
      <div>
        ...
        <div
          className={`${
            cart ? "cartColorAttribute" : "headerCartColorAttribute"
          }`}>
          {attribute.items.map((item) => {
            return (
              <div
                key={item.id}
                className={`${cart ? "cartColorItem" : "headerCartColorItem"} ${
                  item.value === chosenAttributes[attribute.name]
                    ? `${cart ? "cartChosenColor" : "headerCartChosenColor"}`
                    : ""
                }`}>
