    ...
    return (
      <div
        className={`productCard ${
          !product.inStock && "productCard-outOfStock"
        }`}
        onMouseEnter={() => this.setState({ hover: true })}
        onMouseLeave={() => this.setState({ hover: false })}>
        <div
          className={`addToCartSvg ${this.state.hover ? "" : "nonDisplay"}`}
          onClick={product.inStock ? addProduct : undefined}></div>
        <Link to={`/${product.category}/${product.id}`}>
          ...
          {!product.inStock && (
            <p className='productCard-outOfStock-text'>OUT OF STOCK</p>
          )}
        </Link>
      </div>
    );
